...
通过完全复用阅微 App 内置的「设备同步」功能（基于 mDNS + 自定义 TCP 协议），把电脑上的 EPUB 书籍推送到局域网内的安卓阅微设备。**无需改动 App**。PC 端需要先用**邮箱验证码登录**阅微账号（绑定账号后即可使用，不依赖 App 内的登录态）。

## 功能

- **邮箱验证码登录**：PC 端需登录阅微账号后才能启动同步（不依赖 App 内登录态，登录后记住会话）
- **免改动 App**：不改动阅微 App 任何代码，完全复用其「设备同步」协议
...
- **push 到 `main`**：自动构建并发布（更新「最新版」Release，附 macOS/Windows 产物）
- **打 tag**：发布版本化 Release，如 `ReaMicroPC v1.0.0`：
